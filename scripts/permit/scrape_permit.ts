...
const outputFilePath = (chainId: string) =>
...
async function filterForPermits(
  ...
  const tokensWithVersion = await getDomainSeparatorVersion(
    filtered,
    chainId,
    client
  );
  const permitResults = await checkPermit(
    client,
    tokensWithVersion,
    getEvmChain(chainId) as any
  );
  return tokensWithVersion
    .map((token) => ({
      ...token,
      status: permitResults.find((r) => r.tokenAddress === token.token.address)
        ?.status,
      version: permitResults.find((r) => r.tokenAddress === token.token.address)
        ?.version,
    }))
  ...
  try {
    ...
    processedResults.forEach((token) => {
      if (!token.version && token.name && token.domainSeparator) {
        ...
        const v1DomainHash = getDomainHash(
          hashedName,
          hashedVersion1,
          chainId,
          token.token.address
        );
        const v2DomainHash = getDomainHash(
          hashedName,
          hashedVersion2,
          chainId,
          token.token.address
        ...
        if (
          token.domainSeparator.toLowerCase() === v1DomainHash.toLowerCase()
        ) {
          token.version = "1";
        } else if (
          token.domainSeparator.toLowerCase() === v2DomainHash.toLowerCase()
        ) {
          token.version = "2";
        } else {
...
export async function scrapePermits(
  chainId: string,
  startIndex?: number,
  endIndex?: number
) {
...
export async function scrapePermitsForTokens(
  ...
  if (!isRpcHealthy) {
    console.log(
      `⚠️  Skipping chain ${chainId} due to RPC health check failure`
    );
    return {};
  }
